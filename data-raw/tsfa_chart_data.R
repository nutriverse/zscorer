...
get_tsfa_zchart <- function(baseurl = "http://www.who.int/childgrowth/standards/",
  ...
  #
  #
  tsfa_chart <- tidyr::gather(data = temp, key = "sd_type", value = "sd_value", names(temp)[4]:names(temp)[ncol(temp)])
  names(tsfa_chart) <- c("sex", "month", "day", "sd_type", "sd_value")
  tsfa_chart$sd_type <- factor(tsfa_chart$sd_type, levels = c("4SD", "3SD", "2SD", "1SD", "0", "-1SD", "-2SD", "-3SD", "-4SD"))
  #
  #
  #
  return(tsfa_chart)
...
get_tsfa_pchart <- function(baseurl = "http://www.who.int/childgrowth/standards/",
  ...
  #
  #
  tsfa_chart <- tidyr::gather(data = temp, key = "p_type", value = "p_value", names(temp)[7]:names(temp)[ncol(temp)])
